...
**Release boundary:** this checkout contains candidate contract changes that are not deployed at the currently recorded address. The historical deployment and production frontend remain listed for provenance, but must not be used to verify the candidate evidence fields. Candidate deployment, schema/readback verification, frontend address wiring and production publish are pending a human-controlled Studionet deployment signature. Live lifecycle transactions are reserved for the user and remain pending.
...
Candidate deployment and frontend publication are pending. Do not use the previous address with candidate UI code. The candidate live economic lifecycle—including evidence measurement, exception adjudication, challenge, settlement, credits and withdrawals—has not yet been demonstrated; see [docs/LIVE_DEMO.md](docs/LIVE_DEMO.md). A deployment and a green CI run are not evidence that those lifecycle transactions occurred.
