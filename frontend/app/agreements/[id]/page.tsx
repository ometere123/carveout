      ...
      <main className="incident-panel">
        ...
        {incident && <>
          ...
          <div className="incident-facts timestamp-facts">
            ...
            <div><span>Exception decision · WAT</span><b>{Number(incident.adjudicated_at||0)>0?<WatTime value={incident.adjudicated_at}/>:(incident.exception_result||"Pending")}</b></div>
          ...
          {incident.measurement_observation_digest&&<div className="basis">Leader observation digest · {incident.measurement_observation_digest}</div>}
          ...
          {incident.exception_observation_digest&&<div className="basis">Leader observation digest · {incident.exception_observation_digest}</div>}
          ...
          {incident.challenge_observation_digest&&<div className="basis">Leader observation digest · {incident.challenge_observation_digest}</div>}
          {(incident.measurement_evidence_record||incident.exception_evidence_record||incident.challenge_evidence_record)&&<details className="evidence-record"><summary>Structured evidence manifest · bounded records</summary><div className="evidence-record-body">{[["Measurement",incident.measurement_evidence_record],["Exception",incident.exception_evidence_record],["Challenge",incident.challenge_evidence_record]].filter(([,record])=>record).map(([label,record]:any)=><section key={label}><div className="kicker">{label} consensus manifest and accepted leader facts</div><pre>{JSON.stringify(JSON.parse(record),null,2)}</pre></section>)}</div></details>}
