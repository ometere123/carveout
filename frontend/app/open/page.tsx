...
export default function Open(){
  ...
  async function submit(){
    ...
    try{
      ...
      for(const group of ["measurement","exception","challenge"]){
        ...
        if(rows.some((x:any)=>x.retrieval_mode!==undefined&&!['REQUEST_JSON','REQUEST_TEXT','RENDER_TEXT'].includes(String(x.retrieval_mode).toUpperCase())))issues.push(group+" policy has an unsupported retrieval_mode; use REQUEST_JSON, REQUEST_TEXT, or RENDER_TEXT.");
      }
  ...
  return <section className="shell page">
    ...
    <div className="form-grid"><div className="form-sheet">
      ...
      <Area label="Source families, origins, paths, and retrieval modes (JSON)" placeholder={'{"measurement":[{"kind":"PROVIDER_STATUS","host":"status.your-service.com","path_prefix":"/incidents","retrieval_mode":"REQUEST_JSON"},{"kind":"INDEPENDENT_PROBE","host":"probe.your-service.org","path_prefix":"/","retrieval_mode":"REQUEST_TEXT"}],"exception":[{"kind":"PUBLIC_NOTICE","host":"notices.your-service.org","path_prefix":"/","retrieval_mode":"RENDER_TEXT"}],"challenge":[{"kind":"COUNTER_EVIDENCE","host":"evidence.your-service.org","path_prefix":"/","retrieval_mode":"RENDER_TEXT"}]}'} value={form.sourcePolicy} set={v=>update("sourcePolicy",v)}/>
      <p className="micro-note">Use real, public HTTPS origins relevant to this service. Each policy group requires 1–8 distinct origins. Measurement needs at least two source families, including an independent probe on a separate origin. retrieval_mode is frozen into the agreement: REQUEST_JSON and REQUEST_TEXT use the stable GenVM HTTP request API; RENDER_TEXT uses the page text renderer. Missing retrieval_mode defaults to RENDER_TEXT. Consensus compares normalized historical service facts, not volatile page metadata. The customer accepts this exact frozen policy. The contract requires at least 10 minutes before SLA start at execution; this app requires you to select at least 15 minutes when creating a proposal to leave time for wallet signing, submission, and finalization. The timestamp uses your selected interval without added minutes. Customer acceptance must still occur at least 5 minutes before SLA exposure.</p>
