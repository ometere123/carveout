...
import { assertWriteWallet } from "./walletGuard";
assertReleaseConfig();
...
export async function write(address:string,functionName:string,args:any[]=[],value?:bigint){
  ...
  const base:any={account:address as `0x${string}`,address:requireContract(),functionName,args,value:value??0n};
  return client.writeContract(base);
}
