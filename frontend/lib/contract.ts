...
export function requireContract(){ if(!CONTRACT_ADDRESS) throw new Error("CARVEOUT contract is not configured yet"); return CONTRACT_ADDRESS as `0x${string}`; }
...
export async function write(address:string,functionName:string,args:any[]=[],value?:bigint){const client=writeClient(address);const base:any={account:address as `0x${string}`,address:requireContract(),functionName,args,value:value??0n};return client.writeContract(base);}
