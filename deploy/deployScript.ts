...
export default async function main(client:GenLayerClient<any>){
  ...
  writeFileSync(path.resolve(process.cwd(),"deployments/studionet.json"),JSON.stringify({product:"CARVEOUT",network:"studionet",chainId:EXPECTED_CHAIN,rpc:EXPECTED_RPC,explorer:"https://explorer-studio.genlayer.com",source:{commit:sourceCommit,path:"contracts/carveout.py",sha256:sourceSha256},constructorArgs:[],deployedAt:new Date().toISOString(),deployment:{status:receipt.statusName,executionResult:receipt.txExecutionResultName,address,txHash:tx}},null,2)+"\n");
  writeFileSync(path.resolve(process.cwd(),"frontend/.env.local"),[`NEXT_PUBLIC_CARVEOUT_CONTRACT=${address}`,`NEXT_PUBLIC_GENLAYER_CHAIN_ID=${EXPECTED_CHAIN}`,`NEXT_PUBLIC_GENLAYER_RPC_URL=${EXPECTED_RPC}`,`NEXT_PUBLIC_GENLAYER_EXPLORER=https://explorer-studio.genlayer.com`,""].join("\n"));
  console.log("CARVEOUT finalized on Studionet:",address); console.log("tx:",tx);
}
