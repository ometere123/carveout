...
The deployed `0.2.0-studionet` contract at `0xA9C86FF6113187915C1Bd8e958fC718719337531` remains the current deployed address, but it does not include the new generic evidence-manifest consensus behavior. The 0.3.0 contract source changes consensus logic and must be deployed as a new release before the revised frontend or a new lifecycle is used. The earlier `0x75f2e473E6f010B510F1d281C8E4679fD2043054` contract is historical. The 0.2.0 live measurement attempt is recorded as UNDETERMINED / `MEASUREMENT_PENDING`, not as a successful measurement and not to be retried.
