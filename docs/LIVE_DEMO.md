...
This is a user-operated browser-wallet runbook. The agent must not sign or submit any of these application writes. The current repository changes the contract's evidence record and neutral-unavailability terminal path; **first deploy this exact candidate and verify its new address/source/schema**. Do not use the previous contract at `0x75f2e473E6f010B510F1d281C8E4679fD2043054` with the new frontend. After deployment, configure the canonical frontend to the returned address and wait for production readiness before this runbook.
